...
auto jacobi_from_pair_distances_unordered(FP r_ab, FP r_ac, FP r_bc) -> JacobiPoint<FP>
{
    ...
    return jacobi_from_pair_distances(r01, r02, r12);
}
...
public:
    ...
    auto operator()(FP dist01, FP dist02, FP dist12) const noexcept -> FP
    {
        ...
        if (dist01 > info.r_upper_limit || dist02 > info.r_upper_limit || dist12 > info.r_upper_limit) {
            return FP {0.0};
        }
        ...
        if (r01 > info.r_shortest_lower_limit) {
            return FP {0.0};
        }
