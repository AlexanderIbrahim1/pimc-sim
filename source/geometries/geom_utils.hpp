...
constexpr void check_lattice_constant_is_positive(FP lattice_constant)
{
    if (lattice_constant <= FP {0.0}) {
        const auto msg =
            std::format("The lattice constant must be positive. Found: {}\n", lattice_constant);
        throw std::runtime_error(msg);
    }
}

template <std::size_t NDIM>
constexpr void check_unit_cell_translations_are_positive(
    const std::array<std::size_t, NDIM>& translations
)
{
