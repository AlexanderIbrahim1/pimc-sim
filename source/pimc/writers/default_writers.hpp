...
static auto default_bisection_multibead_position_move_success_writer(const std::filesystem::path& output_dirpath)
    -> common::writers::BlockValueWriter<std::uint64_t, std::uint64_t>
{
    ...
    return common::writers::BlockValueWriter<std::uint64_t, std::uint64_t> {filepath, header};
}
...
static auto default_bisection_multibead_position_move_info_writer(const std::filesystem::path& output_dirpath)
    -> common::writers::BlockValueWriter<FP, std::uint64_t>
{
    ...
    return common::writers::BlockValueWriter<FP, std::uint64_t> {filepath, header};
}
