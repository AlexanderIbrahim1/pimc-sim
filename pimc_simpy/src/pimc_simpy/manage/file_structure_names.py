...
def _get_slurm_bash_filename(info: ProjectInfo, sim_id: int) -> str:
    return f"{info.subproject_name}_{format_sim_id(sim_id)}.sh"


def _get_abs_simulations_dirpath(info: ProjectInfo) -> Path:
    return info.abs_subproject_dirpath / "simulations"


def _get_abs_slurm_dirpath(info: ProjectInfo) -> Path:
    return info.abs_subproject_dirpath / "slurm"


def _get_abs_slurm_bashfiles_dirpath(info: ProjectInfo) -> Path:
    return _get_abs_slurm_dirpath(info) / "slurm_bashfiles"


def _get_abs_slurm_output_dirpath(info: ProjectInfo) -> Path:
    return _get_abs_slurm_dirpath(info) / "slurm_output"


def _get_abs_simulations_job_dirpath(info: ProjectInfo, sim_id: int) -> Path:
    return _get_abs_simulations_dirpath(info) / f"job_{format_sim_id(sim_id)}"


def get_abs_simulations_job_output_dirpath(info: ProjectInfo, sim_id: int) -> Path:
    return _get_abs_simulations_job_dirpath(info, sim_id) / "output"


def get_abs_slurm_output_filename(info: ProjectInfo, sim_id: int) -> str:
...
def mkdir_job_and_output_dirpaths(info: ProjectInfo, sim_id: int) -> None:
